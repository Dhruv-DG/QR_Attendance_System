...
import 'package:qr_attendance/screens/lecturer/auth/lecturer_signup_screen.dart';
  ...
  Future<void> _loginLecturer() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        final doc = await FirebaseFirestore.instance.collection('lecturers').doc(user.uid).get();
        if (doc.exists && doc['status'] == 'approved') {
          ...
          await _auth.signOut();
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Account not approved yet. Please wait for admin approval.")),
          );
        }
      }
              ...
              child: CircleAvatar(
                ...
                child: ClipOval(
                  child: Image.asset(
                    ...
                    width: 112,
                    height: 112,
                    errorBuilder: (context, error, stackTrace) => Icon(Icons.person, size: 70, color: Colors.white),
                  ),
                ),
              ),
            ),
